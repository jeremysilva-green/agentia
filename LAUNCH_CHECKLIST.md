...
- [x] **Not yet pushed:** user noticed the Ciudad dropdown on `/panel/perfil` renders expanded-but-empty (a blank green box, no list) — confirmed this is exactly the empty-`sifen_cities` gap above, not a separate bug. Decided to leave the dropdown non-functional for now rather than hide it (it'll populate once real city data is seeded), but removed the "Usada para generar tu factura electrónica..." helper text underneath it and simplified the label from "Ciudad (facturación)" to plain "Ciudad" in `AgentProfileForm.tsx`.
...
## 31. "Crear propiedad" silently did nothing when Ciudad was left empty — ✅ pushed

User reported the create-property button appearing to do nothing; self-diagnosed it as a missing city before I could investigate further, but asked for a visible fix so this doesn't happen again silently.

- [x] Root cause: the Ciudad `SingleSelectDropdown` was the only field in `PropertyForm.tsx` missing its `error={fieldError("city")}` prop — every other field had it wired correctly. On top of that, the generic top-level error message was suppressed whenever *any* field error existed (`&& !state.fieldErrors`), so a Ciudad-only validation failure produced zero visible feedback anywhere on the page.
- [x] Fixed both: added the missing `error` prop, and stopped suppressing the generic message when field errors exist.

## 32. Redes Sociales image preview/download crash on large cover photos — ✅ pushed, three-layer fix

User reported the property-card thumbnail broken and the download button not working in the Redes Sociales panel (agent and affiliate — both share the same `/api/property-card/[propertyId]` route and `DownloadPromoCardButton`). Root cause chain, found one layer at a time via real Vercel function logs (local `next dev`/`next build` masked all three):

- [x] **Layer 1:** the route (and the sibling `/api/social-images/generate` pipeline) embedded the raw fetched cover photo as an unresized base64 data URI into Satori's renderer. A large source photo (confirmed: an 8.9MB PNG) blew past Satori's internal SVG parser buffer limit, 500ing the request. Fixed by resizing/recompressing to a capped JPEG via a new `sharp` dependency (`src/lib/serverImage.ts`) before embedding, in both files.
- [x] **Layer 2 (regression, caught before merge):** shipping `sharp` broke the route for *every* property on the deployed preview, not just the large one — `next build`/`next start` worked fine locally, but Vercel's function logs showed `ERR_DLOPEN_FAILED: libvips-cpp.so.8.18.6: cannot open shared object file`. `serverExternalPackages` (already used for `@resvg/resvg-js`/`satori`) stops Turbopack from inlining `sharp`, but Turbopack's output tracing doesn't follow `sharp`'s runtime `dlopen` of the separate `@img/sharp-libvips-linux-x64` package, so that `.so` never shipped with the deployed function. Fixed with `outputFileTracingIncludes` in `next.config.ts`, scoped to the two affected routes.
- [x] **Layer 3:** with loading fixed, the original large-photo property *still* 500'd — with zero log output (a platform-level kill, not a JS exception), while `sharp` resized the exact same file in 75ms locally. Likely cause: fetching the full 8.9MB file over the network plus render work pushed past Vercel's default function timeout. Fixed with `export const maxDuration = 30` on both routes.
- [x] Verified end-to-end on the live preview deployment: the reported property (thumbnail *and* full-size/download path) and three other properties with normal-sized photos all return 200.
- Caught and reverted one bad intermediate deploy (Layer 2) before it could reach production — confirms the value of testing against the actual deployed preview URL, not just `next dev`, before calling an image-pipeline fix done.

## 33. Supabase security advisor: `invoice_counters` publicly accessible — ✅ fixed

Real security email from Supabase flagged `invoice_counters` (added in §26's migration) as having RLS disabled — anyone with the project URL could read/edit/delete it via the REST API. No personal data at risk (just the invoice-numbering counter), but a real fiscal-integrity issue once invoicing goes live: someone could reset the counter and cause duplicate invoice numbers.

- [x] Migration `0053`: `alter table public.invoice_counters enable row level security;`, no policies added — the only legitimate access is through `increment_invoice_number()`, a `security definer` function that bypasses RLS regardless. Verified live: the security-advisor query now returns zero public tables without RLS, and the counter function still increments correctly (bumped the real counter 1→2 during verification — harmless, no real invoices exist yet).

## 34. Billing cut over from Pagopar to Bancard; dLocal removed; Resend invoice email added

User decided to drop Pagopar (blocked on a Ueno Bank business account they can't open for 6 more months) in favor of Bancard (no such bank restriction). Investigated before touching code, per the user's own migration brief: found Bancard was already ~80% built from an earlier evaluation this session (before "Pagopar only" was decided) — `bancard.ts`, `bancardSubscription.ts`, the Bancard webhook, and its recurring-charge cron all existed, just never activated. Also found and corrected several wrong assumptions in the user's brief: Make.com has nothing to do with billing (only Instagram); no automated WhatsApp invoice delivery ever existed to "replace"; Resend was never actually integrated despite the brief assuming it was; zero real subscribers exist to migrate (only one payment ever reached `approved`, and it was a Pagopar sandbox test card).

- [x] `PricingPlans.tsx` and the past-due retry button now call `/api/checkout/bancard` instead of Pagopar's edge function. Pagopar's code (Edge Functions, `PagoparCardForm`/`PagoparCheckoutButton`, migrations `0019`/`0020`) is left in place untouched — rollback-window safety, per the user's own brief.
- [x] dLocal removed entirely (app code, `vercel.json` cron entry) — was dormant, never activated. DB columns/migrations left in place as harmless history.
- [x] New Resend-based invoice email (`src/lib/email/sendInvoiceEmail.ts`), fired from the existing `invoice-status` cron the moment an invoice is confirmed `approved`. Migration `0054` adds `invoice_email_sent_at`/`invoice_email_error` tracking. No-ops cleanly (logs + skips) while `RESEND_API_KEY` is unset.
- [x] **Debit-card safeguard**: confirmed via Bancard's own official checkout-js docs that a debit-card `alias_token` charge requires mounting a client-side PIN-pad widget (`Bancard.Confirmation.loadPinPad`) — something our server-side recurring-charge cron can never supply, so it can never silently complete a debit charge. Added a `needsConfirmation` signal (`bancard.ts`) and a clear agent-facing message (migration `0055` adds `payments.error_message`) instead of a generic decline, plus an upfront warning on the card-save form telling agents to use a credit card.
- [ ] **Still needed before this actually processes a real charge**: real `BANCARD_PUBLIC_KEY`/`BANCARD_PRIVATE_KEY` (currently empty, `BANCARD_ENV=staging`) and a real `RESEND_API_KEY` with a verified sending domain for `facturacion@agentia.com.py`.
- [ ] **Flagged, not resolved**: `bancard.ts`'s `formatAmount()` sends 2-decimal amount strings, matching Bancard's documented "Compra Simple v0.3.1" spec — but the user's own migration brief describes "VPOS 2.0" needing decimal-free integer strings instead. These may be different API products. Left as a loud comment to verify against the real merchant portal before the first real charge, not guessed.

## 35. First production deploy of the session — `main` synced with `preview`

User needed to give Bancard a real, live webhook URL for merchant onboarding, which meant production needed everything `preview` had (44 commits' worth — main hadn't been touched all session). Confirmed scope explicitly before pushing (all of preview vs. just the Bancard commits) — user chose all of it. Clean fast-forward, no divergent production-only history to reconcile.

- [x] `main` pushed to match `preview`'s tip exactly, via a direct branch-to-branch push (`git push origin origin/preview:main`) rather than checking out `main` locally, so the still-held-back local `AgentProfileForm.tsx`/checklist edits were never at risk of being swept in.
- [x] Spot-checked production after deploy: homepage, agent portfolio page, `/panel/suscripcion` auth-redirect, and `/api/checkout/bancard` all healthy. Two things that looked like regressions during verification (`property-card` 500s, a `checkout/bancard` 400) both turned out to be testing artifacts (deployment cold-start blip; a curl quoting issue) — confirmed by retrying and by comparing against `preview`'s known-good behavior, not just assumed away.

## 36. Share links resolving to a dead `vercel.app` URL with no image — ✅ fixed

Real user report: a WhatsApp share showed a `agentia-<hash>-jeremys-projects....vercel.app` domain instead of `agentia.com.py`, with no image preview. Root cause was a previously-known, previously-flagged-but-never-fixed gap: `getSiteUrl()` falls back to Vercel's auto-injected `VERCEL_URL` — a hostname unique to each individual deployment — whenever `NEXT_PUBLIC_SITE_URL` isn't set. It was never actually set for the Production environment in Vercel.

- [x] User set `NEXT_PUBLIC_SITE_URL=https://agentia.com.py` for the Production environment in Vercel, then had it redeployed (an empty commit pushed to `main`, since no code needed to change) to pick it up. Verified: a property page's `og:image`/`og:url` now correctly resolve to `agentia.com.py`.
- [ ] Worth double-checking the **Preview** environment also has its own `NEXT_PUBLIC_SITE_URL` set to the stable preview alias — flagged, not independently re-verified this pass.

## 37. `property-card` OG image had zero caching — every request took 3+ seconds — ✅ fixed

Found while debugging why a share's image never showed up in WhatsApp's preview even after §36's fix. The route (DB lookup + fetch/resize the cover photo + Satori/resvg render) had `Cache-Control: public, max-age=0, must-revalidate` — no caching at all, so *every* request re-ran the full expensive pipeline, including repeat crawler fetches of the exact same URL. Very plausible cause of a crawler timing out before the image loads.

- [x] Added `Cache-Control: public, s-maxage=3600, stale-while-revalidate=86400` to the `ImageResponse`. Verified on production: repeat requests to the same URL now return in ~0.3s (`x-vercel-cache: HIT`) instead of 3+ seconds. Note: the *first* fetch of a brand-new share is still ~3s uncached — if that alone still trips up a crawler on the very first attempt, the real fix would be pre-rendering at save-time instead of on-demand (like the other social-image pipeline already does).

## 38. Property descriptions with `\r\n` line endings corrupted the OG card layout — ✅ fixed

Real user report, with a screenshot: a property's description text overflowed past the card's right edge, with dropped spaces ("Lapropiedad") and misaligned wrapped lines. Root-caused properly rather than guessed at — first tried an `alignSelf`/explicit-`width` theory that had **zero effect** (confirmed via a stale local server process that had been silently serving old builds through several rebuild attempts — caught by directly diffing the compiled output, not just assuming a rebuild worked). Actually root-caused by reproducing the exact bug in an isolated standalone Satori script fed the real database description text: stray `\r` characters (from `\r\n` line endings, likely pasted from Windows/Word) corrupt Satori's text-wrapping badly.
